...
resource "azurerm_network_security_group" "vmnic_nsg" {
  name                = "${azurerm_network_interface.linuxvm_nic.name}-nsg"
...
resource "azurerm_network_interface_security_group_association" "vmnic_nsg_associate" {
  ...
  network_security_group_id = azurerm_network_security_group.vmnic_nsg.id
}
...
resource "azurerm_network_security_rule" "vmnic_nsg_rule_inbound" {
  ...
  resource_group_name         = azurerm_resource_group.rg.name
  network_security_group_name = azurerm_network_security_group.vmnic_nsg.name
}